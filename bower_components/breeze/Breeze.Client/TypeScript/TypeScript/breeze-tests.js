...
function test_entityKey() {
    ...
    var empTerrKey = new breeze.EntityKey(empTerrType, [
        1, 
        77
    ]);
    var empType = em1.metadataStore.getEntityType("Employee");
...
function test_metadataStore() {
    ...
    var ms = new breeze.MetadataStore();
    ms.fetchMetadata("api/NorthwindIBModel").then(function (rawMetadata) {
    }).fail(function (exception) {
    });
...
function test_entityManager() {
    ...
    var emp3 = em1.fetchEntityByKey("Entityee", [
        6
    ]);
    var custType = em1.metadataStore.getEntityType("Customer");
    ...
    var changedCustomersAndOrders = em1.getChanges([
        custType, 
        orderType
    ]);
    ...
    var custType = em1.metadataStore.getEntityType("Customer");
    var orderType = em1.metadataStore.getEntityType("Order");
    var customersAndOrders = em1.getChanges([
        custType, 
        orderType
    ]);
    var custType = em1.metadataStore.getEntityType("Customer");
...
function test_localQueryComparisonOptions() {
    ...
    lqco.setAsDefault();
    var ms = new breeze.MetadataStore({
        localQueryComparisonOptions: lqco
    });
    var em = new breeze.EntityManager({
        metadataStore: ms
    });
...
function test_validator() {
    ...
    freightProperty.validators.push(numericRangeValidator({
        min: 100,
        max: 500
    }));
    ...
    var v0 = breeze.Validator.maxLength({
        maxLength: 5,
        displayName: "City"
    });
    ...
    regionProperty.validators.push(breeze.Validator.maxLength({
        maxLength: 5
    }));
    var orderType = em1.metadataStore.getEntityType("Order");
    var freightProperty = orderType.getProperty("Freight");
    ...
    var custType = em1.metadataStore.getEntityType("Customer");
    var regionProperty = custType.getProperty("Region");
    regionProperty.validators.push(breeze.Validator.stringLength({
        minLength: 2,
        maxLength: 5
    }));
    var validator = breeze.Validator.maxLength({
        maxLength: 5,
        displayName: "City"
    });
